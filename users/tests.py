...
class SubscriptionTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(
            email="user1@test.com", password="testpass123", first_name="User", last_name="One"
        )

        self.user2 = User.objects.create_user(
            email="user2@test.com", password="testpass123", first_name="User", last_name="Two"
        )

        self.moder_group = Group.objects.create(name="moders")
        self.moder_user = User.objects.create_user(email="moder@test.com", password="testpass123")
        self.moder_user.groups.add(self.moder_group)

        # Создаем курсы
        self.course1 = Course.objects.create(
            name="Python Course", description="Learn Python", owner=self.user2  # Владелец - user2
        )

        self.course2 = Course.objects.create(name="Java Course", description="Learn Java", owner=self.user1)

        # Создаем подписку для user1 на course1
        self.subscription = Subscription.objects.create(user=self.user1, course=self.course1)

        # URL для работы с подписками
        self.subscriptions_url = reverse("users:subscriptions")

        # Данные для создания/удаления подписки
        self.subscription_data = {"course_id": self.course2.id}
    ...
    def test_subscription_create_authenticated(self):
        ...
        self.assertTrue(Subscription.objects.filter(user=self.user2, course=self.course2).exists())
    ...
    def test_subscription_delete_existing(self):
        ...
        response = self.client.post(self.subscriptions_url, {"course_id": self.course1.id})
    ...
    def test_subscription_list_pagination(self):
        ...
        courses = []
        for i in range(10):
            course = Course.objects.create(name=f"Course {i}", owner=self.user1)
...
class SubscriptionPermissionsTestCase(APITestCase):
    ...
    def test_user_cannot_manage_subscriptions_for_other_users(self):
        ...
        response = self.client.post(self.subscriptions_url, {"course_id": self.course.id})
...
class StripeServiceTestCase(TestCase):
    ...
    def test_create_product(self, mock_product_create):
        ...
        product = StripeService.create_product(name="Test Course", description="Test Description")

        self.assertEqual(product.id, "prod_test123")
        mock_product_create.assert_called_once_with(name="Test Course", description="Test Description", type="good")
    ...
    def test_create_price(self, mock_price_create):
        ...
        price = StripeService.create_price(amount=100.00, currency="rub", product_id="prod_test123")

        self.assertEqual(price.id, "price_test123")
        mock_price_create.assert_called_once_with(unit_amount=10000, currency="rub", product="prod_test123")
...
class PaymentIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.course = Course.objects.create(
            name="Test Course", description="Test Description", owner=self.user  # Владелец - текущий пользователь
        )
    ...
    def test_create_payment_with_stripe(self, mock_session, mock_price, mock_product):
        ...
        data = {"paid_course": self.course.id, "amount": 1000.00, "payment_method": "card"}
